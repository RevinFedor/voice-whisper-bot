...
export class AiTagsService {
  ...
  async generateTags(noteId: string, customPrompt?: string) {
    ...
    const excludeInstruction = currentNoteTags.length > 0 
      ? `- ВАЖНО: НЕ предлагай теги которые УЖЕ есть в заметке: ${currentNoteTags.join(', ')}`
      : '';
...
- Предложи от 5 тегов (минимум 5, можно больше)
